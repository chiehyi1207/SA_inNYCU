...
public class MemberHelper {
    ...
    public JSONObject getAll() {
        ...
        try {
            ...
            while(rs.next()) {
                ...
                int login_date = rs.getInt("login_date");
                String credit_card = rs.getString("credit_card");
                
                /** 將每一筆會員資料產生一名新Member物件 */
                m = new Member(member_id, email, phone, password, name, login_date, credit_card);
    ...
    public boolean checkLogin(String email, String password ){
        ...
        try {
            ...
            /** 關閉連線並釋放所有資料庫相關之資源 **/
            DBMgr.close(rs, pres, conn); //否則DB會報調，因為通道數目固定
        }
        /** 
         * 判斷是否已經有一筆該電子郵件信箱之資料
         * 若無一筆則回傳False，否則回傳True 
    ...
    public JSONObject create(Member m) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "INSERT INTO `missa`.`members`(`name`, `email`, `phone` , `password`,  `login_date`, `credit_card`)"
                    + " VALUES( ?, ?, ?, ?, ?, ?)";
            ...
            int login_date = m.getLoginDate();
            String credit_card = m.getCreditCard();
            ...
            pres.setInt(5, login_date);
            pres.setString(6, credit_card);
    ...
    public JSONObject update(Member m) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "Update `missa`.`members` SET `name` = ? ,`phone` = ? ,`password` = ? ,`credit_card` = ? ,`login_date` = ?  WHERE `email` = ?";
            /** 取得所需之參數 */
            String name = m.getName();
            String email = m.getEmail();
            String password = m.getPassword();
    ...
    public void updateLoginDate(Member m) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "Update `missa`.`members` SET `login_date` = ? WHERE `id` = ?";
            /** 取得會員編號 */
            int id = m.getID();
