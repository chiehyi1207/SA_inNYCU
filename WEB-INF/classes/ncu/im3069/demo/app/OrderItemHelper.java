...
public class OrderItemHelper {
    ...
    public JSONArray createByList(long order_id, List<OrderItem> orderproduct) {
        ...
        for(int i=0 ; i < orderproduct.size() ; i++) {
            ...
            /** 取得所需之參數 */
            int cruises_id = op.getCruises().getID();
            ...
            try {
                ...
                /** SQL指令 */
                String sql = "INSERT INTO `missa`.`order_product`(`order_id`, `cruises_id`, `price`, `quantity`, `subtotal`)"
                        + " VALUES(?, ?, ?, ?, ?)";
                
                /** 將參數回填至SQL指令當中 */
                pres = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);
                pres.setLong(1, order_id);
                pres.setInt(2, cruises_id);
	...
	public ArrayList<OrderItem> getOrderProductByOrderId(int order_id) {
	        ...
	        try {
	            /** 取得資料庫之連線 */
	            conn = DBMgr.getConnection();
	            /** SQL指令 */          //join[_order_product, orders]兩個表with[order_id, id]
	            String sql = "SELECT `cruises`.`id` as 'cruises_id', `cruises`.`name`, `order_product`.`order_id` as 'id', `order_product`.`price`, `order_product`.`quantity`, `order_product`.`subtotal`"
	            		+ "FROM `missa`.`order_product`"
	            		+ "INNER JOIN `orders` ON `orders`.`id` = `order_product`.`order_id`"
	            		+ "INNER JOIN `cruises` ON `cruises`.`id` = `order_product`.`cruises_id`"
	            		+ "WHERE `order_id` = ?";
	            ...
	            while(rs.next()) {
	                ...
	                int cruises_id = rs.getInt("cruises_id");
	                ...
	                op = new OrderItem(order_product_id, order_id, cruises_id, price, quantity, subtotal);
